...
class ConfigManager:
    def __init__(self):
        ...
        logger.info(
            f"ConfigManager initialized. Search paths: {self.config_dirs}"
        )
    ...
    def import_config(self, source_path: str):
        """Copy a configuration file to the user's config directory."""
    ...
    def delete_config(self, config: VpnConfig):
        ...
        if not str(config.path).startswith(str(C.USER_CONFIGS_DIR)):
            logger.warning(
                f"Attempt to delete non-user config denied: {config.path}"
            )
            raise PermissionError(
                "Only configurations in the user directory can be deleted."
            )
        ...
        try:
            ...
            logger.warning(
                "Config to be deleted was not found (already deleted?): "
                f"{config.path}"
            )
        except OSError as e:
